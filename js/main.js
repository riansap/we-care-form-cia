...
let visitedPages = {}; // Track pages where answers should be restored
...
function initSurvey() {
  ...
  // Mark the first page as visited so its state can be restored
...
function renderCurrentSection() {
  ...
  // Restore saved answers for this section only if it was visited/saved
  restoreAnswers(section, currentPage);
...
function goToPage(pageNum) {
  ...
  const goingBack = pageNum < currentPage;

  // Only save current page answers when navigating BACK.
  // Navigating forward will NOT persist the current page state so the next
  // section stays empty until the user explicitly goes back and saves it.
  if (goingBack) {
    saveCurrentAnswers();
    // mark the page we just left as visited so it can be restored later
    visitedPages[currentPage] = true;
  }
...
function nextPage() {
  if (currentPage < totalPages) {
    // Do NOT require answering when moving to the next page.
    // Move forward without saving current state so the next section
    // remains empty unless it was previously visited.
...
function addRadioListeners() {
  ...
  radios.forEach((radio) => {
    radio.addEventListener("change", function () {
      ...
      // Do not persist on every change. Persistence to sessionStorage
      // will be done when the user navigates BACK or on submit.
    });
  });
}
...
function saveCurrentAnswers() {
  ...
  section.questions.forEach((question) => {
    const selected = document.querySelector(
      `input[name="question_${question.number}"]:checked`
    );
    if (selected) {
...
function restoreAnswers(section) {
  // Only restore answers for this section if the page was visited/saved
  const pageIndex = surveyData.sections.indexOf(section) + 1;
  const canRestore = !!visitedPages[pageIndex];
  ...
  section.questions.forEach((question) => {
    ...
    if (savedValue) {
      const radio = document.querySelector(
        `input[name="question_${question.number}"][value="${savedValue}"]`
      );
      if (radio) {
        radio.checked = true;
      }
...
function validateAllSections() {
  ...
  if (!allValid) {
    alert(
      `Pertanyaan berikut belum dijawab: ${unansweredQuestions.join(", ")}`
    );
  }
